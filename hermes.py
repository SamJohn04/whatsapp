...
def readList():
    ...
    read.mainloop()
    fn=open("../App/numbers.txt","a")
    for n in reci:
        fn.write(n)
    fn.close()
    fg=open("../App/groups.txt","a")
    for g in recig:
        fg.write(g)
    fg.close()
def greetbytime():
    ...
    for n in recilist:
        if t.tm_hour<=10:
            sendwhatmsg_instantly(n, 'Good Morning!')
        elif t.tm_hour<13:
            sendwhatmsg_instantly(n,'Good Day!')
        elif t.tm_hour<17:
            sendwhatmsg_instantly(n,'Good Afternoon!')
        else:
            sendwhatmsg_instantly(n,'Good Evening!')
    for g in reciglist:
        if t.tm_hour<=10:
            sendwhatmsg_to_group_instantly(g, 'Good Morning!')
        elif t.tm_hour<13:
            sendwhatmsg_to_group_instantly(g,'Good Day!')
        elif t.tm_hour<17:
            sendwhatmsg_to_group_instantly(g,'Good Afternoon!')
        else:
            sendwhatmsg_to_group_instantly(g,'Good Evening!')

def justGreet():
    getList()
    for n in recilist:
        sendwhatmsg_instantly(n,'Hello!!')
    for g in reciglist:
        sendwhatmsg_to_group_instantly(g, 'Hello!')
...
def somethingElse():
    ...
    def proceed():
        x=e.get()
        for n in recilist:
            sendwhatmsg_instantly(n,x)
        for g in reciglist:
            sendwhatmsg_to_group_instantly(g, x)
        e.delete(0,END)
